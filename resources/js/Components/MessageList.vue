...
<template>
    <div v-show="$page.props.psychologists" class="w-full px-5 flex flex-col justify-between max-h-full">
        <div class="flex flex-col mt-5 h-screen overflow-y-auto">
    ...
    <div v-show="$page.props.clients" class="w-full px-5 flex flex-col justify-between max-h-full">
        <div class="flex flex-col mt-5  h-screen overflow-y-auto">
            <div class="flex mb-4" v-for="message in messages"
    ...
    <div v-show="$page.props.showMessage" class="w-full px-5 flex flex-col justify-between max-h-full">
        <div class="flex flex-col mt-5  h-screen overflow-y-auto">
            <div class="flex mb-4" v-for="message in messages"
                :class="message.model_sender_id == 2 ? 'flex-row-reverse justify-end' : 'justify-end'">
                <div>
...
export default {
    ...
    methods: {
        fetchMessages(orderId) {
            axios.get(`/dashboard/chat/${orderId}/fetch`)
                .then(response => {
        ...
        addMessage(message) {
            ...
            axios.post(`/dashboard/chat/${message.orderId}/send`, message)
                .then(response => {
    ...
    mounted() {
        this.fetchMessages(this.$page.props.order[0].order_id)
